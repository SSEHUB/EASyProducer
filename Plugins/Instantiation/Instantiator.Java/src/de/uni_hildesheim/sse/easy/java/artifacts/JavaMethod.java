...
public class JavaMethod extends JavaParentFragmentArtifact {
    ...
    private static EASyLogger logger = EASyLoggerFactory.INSTANCE.getLogger(JavaMethod.class, Bundle.ID);
     ...
     */
    public ArraySet<JavaCall> statements() {
        final List<JavaCall> list = new ArrayList<JavaCall>();
            ...
            public boolean visit(ExpressionStatement node) {
                if (node.getExpression() instanceof MethodInvocation) {
                    ...
                    if (null != methodInvocation.getExpression()) {
                        ...
                        if (null != typeBinding) {
                            JavaCall javaCall = new JavaCall(node, methodName, typeBinding.getName(), JavaMethod.this);
                            list.add(javaCall);
                        }
                    }
                }
        ...
        return new ArraySet<JavaCall>(list.toArray(new JavaCall[list.size()]), JavaCall.class);
    }
    
    /**
     * Deletes a sinle java call.
     * 
     * @param call the call that should be deleted.
     */
    public void deleteJavaCall(JavaCall call) {
        for (JavaCall javacall : statements()) {
            try {
                if (call.getName().equals(javacall.getName()) && call.getType().equals(javacall.getType())) {
                    javacall.delete();
                }
