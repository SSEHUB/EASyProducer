 ...
 */
public class JavaCodeTypeSpecification implements IJavaCodeElement {
    ...
    protected JavaCodeTypeSpecification(String type, JavaCodeClass enclosing) {
        ...
        if (gStartPos > 0 && gEndPos > 0) {
            ...
            type = type.substring(0, gStartPos).trim();
            generics = parseGenerics(gen);
        }
     ...
     */
    public static List<String> parseGenerics(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            case ',':
                if (nested == 0) {
                    ...
                    result.add(text.substring(lastPos, i).trim());
                    lastPos = i + 1;
                }
                break;
            default:
        ...
        if (lastPos > 0 && lastPos < endGen) {
            ...
            result.add(text.substring(lastPos, endGen).trim());
        }
        return result;
    }
