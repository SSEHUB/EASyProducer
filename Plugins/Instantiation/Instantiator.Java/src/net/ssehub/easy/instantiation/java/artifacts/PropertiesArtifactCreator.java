...
/**
 * Defines the default file artifact creator for property files.
 * 
 * @author Eichelberger
 */
public class PropertiesArtifactCreator extends DefaultFileArtifactCreator {
    
    @Override
    protected boolean handlesFileImpl(File file) {
        return checkSuffix(file, ".properties");
    }
    
    @Override
    public boolean handlesArtifact(Class<? extends IArtifact> kind, Object real) {
        boolean result = false;
        if (PropertiesFileArtifact.class == kind) { // second-level, only support this very specific artifact type here
    ...
    public FileArtifact createArtifactInstance(Object real, ArtifactModel model) throws VilException {
        return new PropertiesFileArtifact((File) real, model);
    }
    
    @Override
    public java.lang.Class<? extends IArtifact> getArtifactClass() {
        return PropertiesFileArtifact.class;
    }
