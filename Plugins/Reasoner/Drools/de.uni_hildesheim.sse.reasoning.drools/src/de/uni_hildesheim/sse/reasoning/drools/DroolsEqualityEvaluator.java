 ...
 *
 */
public class DroolsEqualityEvaluator implements IConstraintTreeVisitor, IValueVisitor {
    ...
    private List<String> contValues = new ArrayList<String>();
    private Number ruleCount = 0;
    private String ruleHeader = "";
    private final String ruleAttributes = "agenda-group " + "\"" + "Group 2" + DroolsVisitor.getImportedProjectName()
            + "\"" + "\n"
            + "dialect " + "\"mvel" + "\""
            + "\nno-loop true" 
            + "\n";
    private final String modificationAttributes = "\n" + "dialect " + "\"mvel" + "\""
            + "\nno-loop true" 
            + "\n";
     ...
     */
    public DroolsEqualityEvaluator() {
     ...
     */
    public List<String> getContValueRules() {
        return contValues;
    }
...
//            logger.info("Non supported rule is " + rule);
    ...
    private void addRuleHeaders() {
        if (!modificationConstraint) {
            ...
            DroolsVisitor.addAgendaGroup2("Group 2" + "_" + ruleCount);
        }
    }
...
//        this.ruleSupported = false;
    ...
    public void visitReferenceValue(ReferenceValue referenceValue) {
        ...
        String refName = referenceValue.getValue().getName();
        int droolsClassNr = getDroolsClassNr(refName);
    ...
    public void visitMetaTypeValue(MetaTypeValue value) {
        ...
        if (!modificationConstraint) {
            ...
            if ((null != li)) {
                String pattern2 = "";
                for (int i = 0; i < li.size(); i++) {
                    String access2 = variableStatusChecker + "(" + "\"" + li.get(i) + "." + currentSlot + "\"" + ")";
                    pattern2 += " || " + access2;
                }
