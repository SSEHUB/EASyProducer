...
(run)
(assert (project (projectName Infra) ))

(defmodule mod2)

(defrule mod2::RULE_1
?id <- (com::SerialConnection (baudRate $?baudRate)(comPort $?comPort)(stopBits $?stopBits)(parity $?parity)(flowControl $?flowControl)(dataBits $?dataBits)(timeOut $?timeOut)) (com::LightControl (noOfXCoordinates $?noOfXCoordinates)(noOfYCoordinates $?noOfYCoordinates)(connectionType $?connectionType)(name $?name)) (com::Scale (connectionType $?connectionType)(scaleType $?scaleType)(resolution $?resolution)) (com::TcpConnection (port $?port)(ipAddress $?ipAddress)(timeOut $?timeOut)) 
(test (not (> )  ) )
=>
