...
public class SimpleDataReceiver extends SpringAsyncServiceBase {
    ...
    public SimpleDataReceiver() {
        service = AbstractService.createInstance("de.iip_ecosphere.platform.test.apps.serviceImpl.SimpleReceiverImpl", 
            SimpleDataReceiverInterface.class, 
            "SimpleReceiver", 
            "deployment.yml");
    ...
    */
    ...
    public void initService() {
        if (null == activated || "".equals(activated) || "true".equals(activated)) {
            LoggerFactory.getLogger(getClass()).info("Initializing service SimpleReceiver: {}", activated);
            serviceSent = Counter.builder("service.sent")
                .baseUnit("tuple/s")
                .description("Tuples sent out by a service")
                .tags("service", "SimpleReceiver", "application", "SimpleMeshApp", "device", Id.getDeviceId())
            ...
            createReceptionCallback("SimpleSource_Rec1", receiveRec1_SimpleReceiver(), Rec1.class, "receiveRec1_SimpleReceiver-in-0");
        }
    }
