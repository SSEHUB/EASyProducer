...
public class SimpleDataTransformer3 extends SpringAsyncServiceBase {
    ...
    public SimpleDataTransformer3() {
        service = AbstractService.createInstance("de.iip_ecosphere.platform.test.apps.serviceImpl.SimpleTransformer3Impl", 
            SimpleDataTransformer3Interface.class, 
            "SimpleTransformer3", 
            "deployment.yml");
    ...
    */
    ...
    public void initService() {
        if (null == activated || "".equals(activated) || "true".equals(activated)) {
            LoggerFactory.getLogger(getClass()).info("Initializing service SimpleTransformer3: {}", activated);
            serviceSent = Counter.builder("service.sent")
                .baseUnit("tuple/s")
                .description("Tuples sent out by a service")
                .tags("service", "SimpleTransformer3", "application", "SimpleMeshApp3", "device", Id.getDeviceId())
