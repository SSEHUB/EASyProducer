...
@TestPropertySource(properties = {
    "spring.cloud.stream.poller.fixedDelay=800",
        ...
        "spring.cloud.stream.source=data_createKRec13_SimpleKodexSource_KodexMeshApp",
...
public class SimpleKodexDataSourceTest extends SpringAsyncServiceBase {
    ...
    */
    public void testSource() throws IOException {
        ...
        createReceptionCallback("data_SimpleKodexSource_KRec13_KodexMeshApp", d -> {
            incrementReceived(KRec13.class); 
            printReceivedData(d);
            Assert.assertTrue(predKRec13.test(d));
        }, KRec13.class);
        TimeUtils.sleep(5000);
        Service svc = iip.Starter.getMappedService("SimpleKodexSource");
        if (null != svc) {
            try {
                LoggerFactory.getLogger(getClass()).info("Service autostop (test): SimpleKodexSource");
                svc.setState(ServiceState.STOPPING);
            } catch (ExecutionException e) {
                LoggerFactory.getLogger(getClass()).error("Stopping service SimpleKodexSource: {}", e.getMessage());
            }
        }
        System.exit(0);
    }
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        YamlService yaml = YamlArtifact.readFromYamlSafe(ResourceLoader.getResourceAsStream("deployment.yml"))
            .getServiceSafe("SimpleKodexSource");
        File f = FileUtils.findFile(new File(".."), "SimpleKodexTestingApp-0.1.0-SNAPSHOT-bin.jar");
        if (null != f && null != yaml.getProcess()) {
            Starter.extractProcessArtifacts("SimpleKodexSource", yaml.getProcess(), f, null);
        } else {
            LoggerFactory.getLogger(SimpleKodexDataSourceTest.class).info("Service artifact {} not found in {}", "SimpleKodexTestingApp-0.1.0-SNAPSHOT-bin.jar", "..");
        }
