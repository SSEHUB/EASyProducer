package iip.datatypes;

import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonProperty;

import de.iip_ecosphere.platform.services.environment.IipStringStyle;
import de.iip_ecosphere.platform.support.iip_aas.ConfiguredName;

import iip.datatypes.RestPhoenixEEMResponseMeasurementSingle;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

/**
 * Implementation of the application data type RestPhoenixEEMResponseMeasurementSingle.
 * Generated by: EASy-Producer.
 */
@ConfiguredName("RestPhoenixEEMResponseMeasurementSingle")
public class RestPhoenixEEMResponseMeasurementSingleImpl implements RestPhoenixEEMResponseMeasurementSingle {

    @JsonProperty("context")
    @ConfiguredName("context")
    private String context;
    @JsonProperty("id")
    @ConfiguredName("id")
    private String id;
    @JsonProperty("timestamp")
    @ConfiguredName("timestamp")
    private String timestamp;
    @JsonProperty("name")
    @ConfiguredName("name")
    private String name;
    @JsonProperty("value")
    @ConfiguredName("value")
    private Object value;
    @JsonProperty("unit")
    @ConfiguredName("unit")
    private String unit;
    @JsonProperty("description")
    @ConfiguredName("description")
    private String description;

    /**
     * Default constructor. Fields are pre-allocated with default Java values.
     */
    public RestPhoenixEEMResponseMeasurementSingleImpl() {
    }

    /**
     * Copy constructor.
     *
     * @param from the instance to copy the values from
     */
    public RestPhoenixEEMResponseMeasurementSingleImpl(RestPhoenixEEMResponseMeasurementSingle from) {
        this.context = from.getContext();
        this.id = from.getId();
        this.timestamp = from.getTimestamp();
        this.name = from.getName();
        this.value = from.getValue();
        this.unit = from.getUnit();
        this.description = from.getDescription();
    }

    @Override
    @JsonIgnore
    public String getContext() {
        return context;
    }

    @Override
    @JsonIgnore
    public void setContext(String value) {
        this.context = value;
    }

    @Override
    @JsonIgnore
    public String getId() {
        return id;
    }

    @Override
    @JsonIgnore
    public void setId(String value) {
        this.id = value;
    }

    @Override
    @JsonIgnore
    public String getTimestamp() {
        return timestamp;
    }

    @Override
    @JsonIgnore
    public void setTimestamp(String value) {
        this.timestamp = value;
    }

    @Override
    @JsonIgnore
    public String getName() {
        return name;
    }

    @Override
    @JsonIgnore
    public void setName(String value) {
        this.name = value;
    }

    @Override
    @JsonIgnore
    public Object getValue() {
        return value;
    }

    @Override
    @JsonIgnore
    public void setValue(Object value) {
        this.value = value;
    }

    @Override
    @JsonIgnore
    public String getUnit() {
        return unit;
    }

    @Override
    @JsonIgnore
    public void setUnit(String value) {
        this.unit = value;
    }

    @Override
    @JsonIgnore
    public String getDescription() {
        return description;
    }

    @Override
    @JsonIgnore
    public void setDescription(String value) {
        this.description = value;
    }

    @Override
    public int hashCode() {
        int hc = 0;
        hc += getContext() != null ? getContext().hashCode() : 0;
        hc += getId() != null ? getId().hashCode() : 0;
        hc += getTimestamp() != null ? getTimestamp().hashCode() : 0;
        hc += getName() != null ? getName().hashCode() : 0;
        hc += getValue() != null ? getValue().hashCode() : 0;
        hc += getUnit() != null ? getUnit().hashCode() : 0;
        hc += getDescription() != null ? getDescription().hashCode() : 0;
        return hc;
    }

    @Override
    public boolean equals(Object other) {
        boolean eq;
        if (other instanceof RestPhoenixEEMResponseMeasurementSingle) {
            RestPhoenixEEMResponseMeasurementSingle oth = (RestPhoenixEEMResponseMeasurementSingle) other;
            eq = true;
            eq &= getContext() != null ? getContext().equals(oth.getContext()) : true;
            eq &= getId() != null ? getId().equals(oth.getId()) : true;
            eq &= getTimestamp() != null ? getTimestamp().equals(oth.getTimestamp()) : true;
            eq &= getName() != null ? getName().equals(oth.getName()) : true;
            eq &= getValue() != null ? getValue().equals(oth.getValue()) : true;
            eq &= getUnit() != null ? getUnit().equals(oth.getUnit()) : true;
            eq &= getDescription() != null ? getDescription().equals(oth.getDescription()) : true;
        } else {
            eq = false;
        }
        return eq;
    }

    @Override
    public String toString() {
        return ReflectionToStringBuilder.toString(this, IipStringStyle.SHORT_STRING_STYLE);
    }

}
