...
@TestPropertySource(properties = {
    "spring.cloud.function.definition=transformRec13Rec13_SimpleTransformer3",
        "spring.cloud.stream.source=data_createRec13_SimpleSource3_SimpleMeshApp3;data_transformRec13Rec13_SimpleTransformer3_SimpleMeshApp3",
...
public class SimpleDataTransformer3Test extends SpringAsyncServiceBase {
    ...
    public void testService(InputStream in) throws IOException {
        try (ConfigurableApplicationContext context = new SpringApplicationBuilder(
            ...
            consumer.addHandler(Rec13Impl.class, d -> {
                                        ...
                                        Transport.send(c -> c.asyncSend("createRec13_SimpleSource3", d), "SimpleSource3", "createRec13_SimpleSource3-in-0");
                                        // one for async via transport - ignored if spring
                                        Transport.send(c -> c.asyncSend("data_SimpleSource3_Rec13_SimpleMeshApp3", d), "SimpleSource3", "createRec13_SimpleSource3-in-0");
                                    });
            consumer.addHandler(Rec13Impl.class, d -> {
                                        // one for spring - ignored if async
    ...
    */
    ...
    public void testSimpleTransformer3Service() throws IOException {
        ...
        Service svc = iip.Starter.getMappedService("SimpleTransformer3");
        if (null != svc) {
            try {
                LoggerFactory.getLogger(getClass()).info("Service autostop (test): SimpleTransformer3");
                svc.setState(ServiceState.STOPPING);
            } catch (ExecutionException e) {
                LoggerFactory.getLogger(getClass()).error("Stopping service SimpleTransformer3: {}", e.getMessage());
            }
        }
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        if (null != f && null != yaml.getProcess()) {
            ...
            LoggerFactory.getLogger(SimpleDataTransformer3Test.class).info("Service artifact {} not found in {}", "SimpleMeshTestingApp3-0.1.0-SNAPSHOT-bin.jar", "..");
        }
