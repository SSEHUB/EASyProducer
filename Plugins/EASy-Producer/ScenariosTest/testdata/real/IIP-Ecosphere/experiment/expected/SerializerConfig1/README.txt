...
This folder contains the top-level artifacts that are needed to execute the platform (in the following sequence):
  - The communication broker. If possible, we instantiate a working broker into the broker folder. System or 
    ...
    - broker.bat for Windows and JDK >= 11 and JDK <= 15
    - broker.sh for Linux/Bash and JDK >= 11 and JDK <= 15
    - setup in src
    - binaries in "brokerJars"
    - iip-broker.service for Linux/systemd and JDK >= 11 and JDK <= 15
  - The central platform services, in particular the AAS server (due to various components, JDK 8 is not 
    ...
    - platform.bat for Windows and JDK >= 11 and JDK <= 15
    - platform.sh for Linux/Bash and JDK >= 11 and JDK <= 15
    - source code in "platform"
    - binaries in "plJars"
    - iip-platform.service for Linux/systemd and JDK >= 11 and JDK <= 15
  - The central monitoring component (if configured, as long as BaSyx, Tomcat and monitoring do conflict)
    - monitoring.bat for Windows and JDK >= 11 and JDK <= 15
    - monitoring.sh for Linux/Bash and JDK >= 11 and JDK <= 15
    - source code in "monitoring"
    - binaries in "monJars"
    - iip-monitoring.service for Linux/systemd and JDK >= 11 and JDK <= 15
  - The ECS runtime with container manager
    - ecs.bat for Windows and JDK >= 11 and JDK <= 15
    - ecs.sh for Linux/Bash and JDK >= 11 and JDK <= 15
    ...
    - iip-ecs.service (with dependency to platform.service) as well as ecs-noDeps.service (for standalone 
      device installation) for Linux/systemd and JDK >= 11 and JDK <= 15
  - The service manager
    - serviceMgr.bat for Windows and JDK >= 11 and JDK <= 15
    - serviceMgr.sh for Linux/Bash and JDK >= 11 and JDK <= 15
    ...
    - iip-serviceMgr.service (with dependency to ecs.service) as well as serviceMgr-noDeps.service (for standalone 
      device/container installation) for Linux/systemd and JDK >= 11 and JDK <= 15
  - For small resources: ECS runtime without container manager but including service manager
    - ecsServiceMgr.bat for Windows and JDK >= 11 and JDK <= 15
    - ecsServiceMgr.sh for Linux/Bash and JDK >= 11 and JDK <= 15
    - source code in "ecsServiceMgr"
    - binaries in "ecsSvcJars"
    - iip-ecsServiceMgr.service for Linux/systemd and JDK >= 11 and JDK <= 15
  - The platform command line 
    - cli.bat for Windows and JDK >= 11 and JDK <= 15
    - cli.sh for Linux/Bash and JDK >= 11 and JDK <= 15
