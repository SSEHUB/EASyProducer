...
public class ModbusReceiver extends SpringAsyncServiceBase {
    ...
    public ModbusReceiver() {
        service = AbstractService.createInstance("ReceiverImpl", 
            ModbusReceiverInterface.class, 
            "ModbusReceiver", 
            "deployment.yml");
    ...
    */
    ...
    public void initService() {
        if (null == activated || "".equals(activated) || "true".equals(activated)) {
            LoggerFactory.getLogger(getClass()).info("Initializing service ModbusReceiver: {}", activated);
            String iId;
            String sId;
            ...
            createReceptionCallback("data_myModbusConn_ModbusPhoenixEEM_ModbusApp" + appInstId, receiveModbusPhoenixEEM_ModbusReceiver(), ModbusPhoenixEEM.class, "receiveModbusPhoenixEEM_ModbusReceiver-in-0");
            createReceptionCallback("data_myModbusSentronConn_ModbusSiemensSentron_ModbusApp" + appInstId, receiveModbusSiemensSentron_ModbusReceiver(), ModbusSiemensSentron.class, "receiveModbusSiemensSentron_ModbusReceiver-in-0");
        }
    }
