...
/**
 * Implements tests for "KIFamilyExample". The generated class is meant to be re-usable and extensible, e.g., regarding
 ...
 * "../../src/test/resources/testData-myKiFamily.json", the default resource
 * "testData-myKiFamily.json" or "resources/software" if you want to deploy it, e.g., in case of a mocked
 ...
 * in the configuration model. The structure for this service is: {@code {"rec1": {"intField": m,
 * "stringField": o}, "$period": p, "$repeats": r}}. Depending on your data type definitions in the model,
...
"spring.cloud.function.definition=transformRec1RtsaTestInput_myKiFamily",
"spring.cloud.stream.source=data_processRec1_myAnonymizer_myApp;data_transformRtsaTestInput_myKiFamily_myApp",
"iip.service.aasSink=false",
"iip.service.myAnonymizer=false",
"iip.service.myKiFamily=true",
"iip.service.myMqttConn=false",
"iip.service.myOpcConn=false",
"iip.service.myRtsa=false",
"iip.service.mySource=false"})
@RunWith(SpringRunner.class)
public class KIFamilyExampleTest extends SpringAsyncServiceBase {
    ...
    */
    public static class DataUnit extends BaseDataUnit {
        ...
        /**
        * Returns the value of rec1.
        *
        * @return the value of rec1, may be <b>null</b>
        */
        ...
        /**
        * Changes the value of rec1. [required by Jackson]
        *
    ...
    public void testService(InputStream in) throws IOException {
        try (ConfigurableApplicationContext context = new SpringApplicationBuilder(TestChannelBinderConfiguration.
            ...
            consumer.addHandler(Rec1Impl.class, d -> {
                Message<Rec1Impl> msg = MessageBuilder.withPayload(d)
                    .build();
                source.send(msg);
                assertThat(target.receive().getPayload(), matcher);
            });
            final Predicate<RtsaTestInput> predRtsaTestInput = getAssertPredicateRtsaTestInput();
            createReceptionCallback("data_myKiFamily_RtsaTestInput_myApp", d -> {
                incrementReceived(RtsaTestInput.class); 
                printReceivedData(d);
                assertTrue(predRtsaTestInput.test(d));
            }, RtsaTestInput.class);
    ...
    * "../../src/test/resources/testData-myKiFamily.json" or the default resource
    * "testData-myKiFamily.json".
    *
    * @throws IOException if the data cannot be loaded/mapped to the service input data
    */
    public void testService() throws IOException {
        InputStream in;
        try {
            String fileName = System.getProperty("iip.test.dataFile", "../../src/test/resources/testData-myKiFamily.jso"
                + "n");
            LoggerFactory.getLogger(getClass())
                .info("Opening {}", fileName);
            in = new FileInputStream(fileName);
        } catch (IOException e) {
            in = ResourceLoader.getResourceAsStream("testData-myKiFamily.json");
        }
        if (null != in) {
            testService(in);
        } else {
    ...
    */
    ...
    /**
    * Creates/returns a predicate asserting that the received data of type RtsaTestInput as output of the testing object
    ...
    */ 
    protected Predicate<RtsaTestInput> getAssertPredicateRtsaTestInput() {
        return d -> true;
    }
    ...
    */
    ...
    /**
    * Tests the service with the default resource "testData-myKiFamily.json".
    *
    * @throws IOException shall not occur / test failure
    */
    @Test
    public void testMyKiFamilyService() throws IOException {
        ...
        Service svc = Starter.getMappedService("myKiFamily");
        if (null != svc) {
            try {
                LoggerFactory.getLogger(getClass())
                    .info("Service autostop (test): myKiFamily");
                svc.setState(ServiceState.STOPPING);
            } catch (ExecutionException e) {
                LoggerFactory.getLogger(getClass())
                    .error("Stopping service myKiFamily: {}", e.getMessage());
            }
        }
