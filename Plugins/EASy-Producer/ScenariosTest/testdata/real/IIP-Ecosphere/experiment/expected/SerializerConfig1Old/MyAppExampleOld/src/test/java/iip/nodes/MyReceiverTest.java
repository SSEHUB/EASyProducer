...
/**
 * Implements tests for "MyReceiver". The generated class is meant to be re-usable and extensible, e.g., regarding the
 ...
 * "../../src/test/resources/testData-MyReceiver.json", the default resource
 * "testData-MyReceiver.json" or "resources/software" if you want to deploy it, e.g., in case of a mocked
 ...
 * in the configuration model. The structure for this service is: {@code {"string": , "$period": p, "$repeats":
 * r}}. Depending on your data type definitions in the model, individual fields may be mandatory (indicated
 * by <i>m</i>), optional (indicated by <i>o</i>) or nested fields (not indicated further). "$period" is an
 * optional generic entry that defines the delay period <i>p</i> between the actual and the next data entry (if
...
public class MyReceiverTest extends SpringAsyncServiceBase {
    ...
    * "../../src/test/resources/testData-MyReceiver.json" or the default resource 
    * "testData-MyReceiver.json".
    *
    * @throws IOException if the data cannot be loaded/mapped to the service input data
    */
    public void testService() throws IOException {
        InputStream in;
        try {
            String fileName = System.getProperty("iip.test.dataFile", "../../src/test/resources/testData-MyReceiver.jso"
                + "n");
            LoggerFactory.getLogger(getClass())
    ...
    */
    ...
    public void testMyReceiverService() throws IOException {
        ...
        Service svc = Starter.getMappedService("MyReceiver");
        if (null != svc) {
            try {
                LoggerFactory.getLogger(getClass())
                    .info("Service autostop (test): MyReceiver");
                svc.setState(ServiceState.STOPPING);
            } catch (ExecutionException e) {
                LoggerFactory.getLogger(getClass())
                    .error("Stopping service MyReceiver: {}", e.getMessage());
            }
        }
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        if (null != f && null != yaml.getProcess()) {
            ...
            LoggerFactory.getLogger(MyReceiverTest.class)
