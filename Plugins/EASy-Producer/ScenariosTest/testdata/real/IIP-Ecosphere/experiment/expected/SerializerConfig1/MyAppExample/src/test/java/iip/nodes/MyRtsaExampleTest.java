...
/**
 * Implements tests for "MyRtsaExample". The generated class is meant to be re-usable and extensible, e.g., regarding the
 ...
 * "../../src/test/resources/testData-myRtsa.json", the default resource "testData-myRtsa.json" or
 * "resources/software" if you want to deploy it, e.g., in case of a mocked connector. The input data file is a JSON
 * file, one data unit per line, following a generic structure per data unit. A data unit consists of one optional
 * object entry per input type of the service under test (attribute name is the type name with first character
 * in small case) - the contents of the same structure as defined in the configuration model. The structure
 * for this service is: {@code {"rtsaTestInput": {"id": m, "value1": m, "value2": m}, "$period": p,
...
"spring.cloud.function.definition=processRtsaTestInput_myRtsa",
"spring.cloud.stream.source=data_processRtsaTestOutput_myRtsa_myApp;data_transformRtsaTestInput_myKiFamily_myApp",
"iip.service.aasSink=false",
"iip.service.myAnonymizer=false",
"iip.service.myKiFamily=false",
"iip.service.myMqttConn=false",
"iip.service.myOpcConn=false",
"iip.service.myRtsa=true",
"iip.service.mySource=false"})
@RunWith(SpringRunner.class)
public class MyRtsaExampleTest extends SpringAsyncServiceBase {
    ...
    */
    public static class DataUnit extends BaseDataUnit {
        ...
        private RtsaTestInputImpl rtsaTestInput;
        
        /**
        * Returns the value of rtsaTestInput.
        *
        * @return the value of rtsaTestInput, may be <b>null</b>
        */
        public RtsaTestInputImpl getRtsaTestInput() {
            return rtsaTestInput;
        }
        
        /**
        * Changes the value of rtsaTestInput. [required by Jackson]
        *
        * @param rtsaTestInput the new value, may be <b>null</b>
        */
        public void setRtsaTestInput(RtsaTestInputImpl rtsaTestInput) {
            this.rtsaTestInput = rtsaTestInput;
        }
    ...
    */
    public void testService(InputStream in) throws IOException {
        try (ConfigurableApplicationContext context = new SpringApplicationBuilder(TestChannelBinderConfiguration.
            ...
            consumer.addHandler(RtsaTestInputImpl.class, d -> {
                // one for spring - ignored if async
                Transport.send(c -> c.asyncSend("transformRtsaTestInput_myKiFamily", d), "myKiFamily", 
                    "transformRtsaTestInput_myKiFamily-in-0");
                // one for async via transport - ignored if spring
                Transport.send(c -> c.asyncSend("data_myKiFamily_RtsaTestInput_myApp", d), "myKiFamily", 
                    "transformRtsaTestInput_myKiFamily-in-0");
            });
            final Predicate<RtsaTestOutput> predRtsaTestOutput = getAssertPredicateRtsaTestOutput();
            createReceptionCallback("data_myRtsa_RtsaTestOutput_myApp", d -> {
                incrementReceived(RtsaTestOutput.class); 
                printReceivedData(d);
                assertTrue(predRtsaTestOutput.test(d));
            }, RtsaTestOutput.class);
    ...
    * "../../src/test/resources/testData-myRtsa.json" or the default resource 
    * "testData-myRtsa.json".
    *
    * @throws IOException if the data cannot be loaded/mapped to the service input data
    */
    public void testService() throws IOException {
        InputStream in;
        try {
            String fileName = System.getProperty("iip.test.dataFile", "../../src/test/resources/testData-myRtsa.json");
            LoggerFactory.getLogger(getClass())
                .info("Opening {}", fileName);
            in = new FileInputStream(fileName);
        } catch (IOException e) {
            in = ResourceLoader.getResourceAsStream("testData-myRtsa.json");
        }
        if (null != in) {
            testService(in);
        } else {
    ...
    */
    ...
    /**
    * Creates/returns a predicate asserting that the received data of type RtsaTestOutput as output of the testing
    ...
    */ 
    protected Predicate<RtsaTestOutput> getAssertPredicateRtsaTestOutput() {
        return d -> true;
    }
    ...
    */
    ...
    /**
    * Tests the service with the default resource "testData-myRtsa.json".
    *
    * @throws IOException shall not occur / test failure
    */
    @Test
    public void testMyRtsaService() throws IOException {
        ...
        Service svc = Starter.getMappedService("myRtsa");
        if (null != svc) {
            try {
                LoggerFactory.getLogger(getClass())
                    .info("Service autostop (test): myRtsa");
                svc.setState(ServiceState.STOPPING);
            } catch (ExecutionException e) {
                LoggerFactory.getLogger(getClass())
                    .error("Stopping service myRtsa: {}", e.getMessage());
            }
        }
