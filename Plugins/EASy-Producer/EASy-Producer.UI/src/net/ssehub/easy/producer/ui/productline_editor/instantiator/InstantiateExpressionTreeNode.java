...
class InstantiateExpressionTreeNode extends ElementTreeNode<InstantiateExpression> {
    ...
    public String getText() {
        InstantiateExpression ex = getElement();
        String result = "instantiate ";
        if (null != ex.getProject()) {
            result += ex.getProject().getName();
            if (null != ex.getName()) {
                result += " rule " + ex.getName();
            }
        } else {
            result += ex.getQualifiedName();
        }
        result += " ";
        result += argumentsToString(getElement(), 0);
        return result;
    }
